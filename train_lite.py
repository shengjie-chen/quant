...
def train(hyp, opt, device, tb_writer=None):
    ...
    hyp['weight_decay'] *= batch_size * accumulate / nbs  # scale weight_decay 权值衰减
    ...
    if plots:
        plot_labels(labels, names, save_dir) # 根据上面的统计对所有样本的类别，中心点xy位置，长宽wh做可视化
        if tb_writer:
...
if __name__ == '__main__':
    ...
    opt.data, opt.cfg, opt.hyp = check_file(opt.data), check_file(opt.cfg), check_file(opt.hyp)  # check files 检查文件是否存在
