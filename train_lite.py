...
def train(hyp, opt, device, tb_writer=None):
    ...
    """
    logger.info(colorstr('hyperparameters: ') + ', '.join(f'{k}={v}' for k, v in hyp.items()))# 加粗打印超参数信息
    ...
    if pretrained:
        ...
        state_dict = intersect_dicts(state_dict, model.state_dict(), exclude=exclude)  # intersect 似乎是将有序字典变为普通字典
        model.load_state_dict(state_dict, strict=False)  # load
        logger.info('Transferred %g/%g items from %s' % (len(state_dict), len(model.state_dict()), weights))  # report 少加载两个键值对(anchors,anchor_grid)
    else:
    ...
    pg0, pg1, pg2 = [], [], []  # optimizer parameter groups 将模型分成三组(bn.weight,weight,bias)(weight、bn, bias, 其他所有参数)优化
    ...
    mlc = np.concatenate(dataset.labels, 0)[:, 0].max()  # max label class 获取标签中最大的类别值，并于类别数作比较   如果大于类别数则表示有问题
    ...
    if rank in [-1, 0]:
        ...
        if not opt.resume:
            labels = np.concatenate(dataset.labels, 0) # 将所有样本的标签拼接到一起shape为(total, 5)，统计后做可视化
            c = torch.tensor(labels[:, 0])  # classes  获得所有样本的类别
            # cf = torch.bincount(c.long(), minlength=nc) + 1.  # frequency
            # model._initialize_biases(cf.to(device))
            if plots:
                plot_labels(labels, names, save_dir, loggers) # 根据上面的统计对所有样本的类别，中心点xy位置，长宽wh做可视化
                if tb_writer:
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ni = i + nb * epoch  # number integrated batches (since train start) 计算迭代的次数iteration
            ...
            with amp.autocast(enabled=cuda):
                ...
                loss, loss_items = compute_loss(pred, targets.to(device))  # loss scaled by batch_size  # 计算损失，包括分类损失，objectness损失，框的回归损失  # loss为总损失值，loss_items为一个元组，包含分类损失，objectness损失，框的回归损失和总损失
                if rank != -1:
            ...
            # Print
            if rank in [-1, 0]: # 打印显存，进行的轮次，损失，target的数量和图片的size等信息
...
if __name__ == '__main__':
    ...
    # Set DDP variables 分布式数据并行
    # opt.world_size = int(os.environ['WORLD_SIZE']) if 'WORLD_SIZE' in os.environ else 1
    # opt.global_rank = int(os.environ['RANK']) if 'RANK' in os.environ else -1 #global_rank: 多卡训练时卡的数量
